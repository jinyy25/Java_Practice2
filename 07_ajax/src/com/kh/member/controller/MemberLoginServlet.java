...
public class MemberLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//checkbox의 value값을 지정하지 않으면  (check했을때 on, check하지 않았을때 null)
		if(saveId!=null) {
		...
		if(m!=null) {
			//로그인 성공!
			//HttpSession객체에 데이터를 대입하기! 로그인 후에는 HttpSession객체를 이용해서 로그인 여부를 확인함.
		...
		//4. 페이지는 반환 : 새로고침시 다시 로직이 동작하지 않도록 sendRedirect를 이용	
	}
